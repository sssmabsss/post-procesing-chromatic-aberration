...
void GraphicsSystem::init(int window_width, int window_height, std::string assets_folder) {
	...
	screen_space_shader_ = new Shader("data/shaders/screen.vert",
	...
	frame_.initColor(window_width, window_width);
...
void GraphicsSystem::update(float dt) {
	...
	shader_->setUniform(U_SCREEN_SIZE_X, viewport_width_);//dt*10);
	shader_->setUniform(U_SCREEN_SIZE_Y, viewport_height_);
	//shader_->setUniform(EFFECT_INTENSITY, effect_intensity);
